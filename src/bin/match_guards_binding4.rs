...
fn print_tile(tile: Tile) {
    use Tile::*;
    match tile{
        Brick(brick @ BrickStyle::Gray | brick @ BrickStyle::Red) => println!("Brick color is {:?}", brick),
        ...
        _ => () //everything else apart from requirement ignored by using unit type
    }
}
fn main() {
    let tile = Tile::Brick(BrickStyle::Gray);
    print_tile(tile); // Brick color is Gray

    let tile = Tile::Sand;
    print_tile(tile); // Ground Tile

    let tile = Tile::Treasure(TreasureChest{
       content: TreasureItem::Gold,
       amount: 200,
    });
    print_tile(tile); // lots of Gold

    let tile = Tile::Water(Pressure(9));
    print_tile(tile); // Water pressure level: 9
}