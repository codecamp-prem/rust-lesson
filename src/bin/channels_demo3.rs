...
// * Use `cargo test --bin channels_demo3` to test your program to ensure all cases are covered
    ...
    thread::spawn(move || {
       ...
       loop {
            if let Ok(msg) = receiver.recv(){
                match msg {
                    LightMsg::ChangeColor(sent_color) => {
                        println!("Color changed to: {}", sent_color);
                        match light_status {
...
fn main() {
    ...
    s.send(LightMsg::On);
    s.send(LightMsg::ChangeColor("red".to_owned()));
    s.send(LightMsg::ChangeColor("green".to_owned()));
    s.send(LightMsg::ChangeColor("blue".to_owned()));
    s.send(LightMsg::Off);
